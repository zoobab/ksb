...
static int ksb26_socks4hs(struct socket *sock, char *dhost, int dport)
{
	int err = -1;
	int buflen = 0;
	char *buf = NULL;
	...
	for(*scnt = 1, ksb26_aux = ksb26_aux->next; ksb26_aux != &ksb26_slh && *scnt < ksb26_maxsocks ; ksb26_aux = ksb26_aux->next) {
		if(ksb26_aux->wrk == 1) {
			...
			if(ksb26_aux->type == 5) {
				...
				if(ksb26_aux->type == 4) {
					if((err = ksb26_socks4hs(sock, ksb26_aux->ipc, ksb26_aux->port)) > 0) {
						(*scnt)++;
...
//			ksb26_slaux->wrk = 0;
		...
		if(ksb26_slaux->type == 5)
			...
			if(ksb26_slaux->type == 4)
				err = ksb26_socks4hs(sock, ksb26_ntoa(daddr), ntohs(dport));
		if(err <= 0) {
//			ksb26_slaux->wrk = 0;
			KSB26_UP_LISTSEM;
