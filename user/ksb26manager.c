...
#define SOCKS1_CMD "wget -O - -q http://ksb.sourceforge.net/socks/socks5.php |\
	egrep -o \"[0-9]*\\.[0-9]*\\.[0-9]*\\.[0-9]*:[0-9]*\" | sed 's/$/;/' |\
	sed 's/^/#S/' | sed 's/$/5|/'>"
#define SOCKS2_CMD "wget -O - -q http://www.atomintersoft.com/products/alive-proxy/socks5-list/ |\
	egrep -o \"[0-9]*\\.[0-9]*\\.[0-9]*\\.[0-9]*:[0-9]*\" | sed 's/$/;/' |\
	sed 's/^/#S/' | sed 's/$/5|/'>"
...
int clear_socks_list(void)
{
	char cmd[BUFFER_SIZE];
	FILE *fp_dev;
	char buff[BUFFER_SIZE];

	snprintf(cmd, BUFFER_SIZE, "#csl|\n", ksb26_dev);
	if(!(fp_dev = fopen(ksb26_dev, "w"))) {
		fprintf(stderr, "[%s] error: fopen: cannot open '%s' device\n", PNAME, ksb26_dev);
		fclose(fp_dev);
		return -1;
...
int get_socks_list(void)
{
	char cmd[BUFFER_SIZE];
	FILE *fp;
	
	snprintf(cmd, BUFFER_SIZE, "%s %s", SOCKS2_CMD, ksb26_dev);
	system(cmd);
	snprintf(cmd, BUFFER_SIZE, "%s %s", SOCKS1_CMD, ksb26_dev);
	system(cmd);
	return 0;
}
...
int main(int argc, char *argv[])
{
	...
	if(insmod_ksb26() == -1)
		exit(EXIT_FAILURE);
	if(load_thosts_list() == -1)
		exit(EXIT_FAILURE);
	while(1) {
		if(clear_socks_list() == -1)
			exit(EXIT_FAILURE);
		if(load_socks_list() == -1)
			exit(EXIT_FAILURE);
		if(get_socks_list() != 0)
			exit(EXIT_FAILURE);
		sleep(wtime);
